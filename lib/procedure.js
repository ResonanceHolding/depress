...
class Procedure {
  ...
  async enter() {
    try {
      ...
      const { size, concurrency, timeout, queue } = this.application.server.semaphore;
      this.application.console.error({
        msg: 'SERVER semaphore fail',
        error,
        size,
        concurrency,
        timeout,
    ...
    if (this.concurrency) {
      try {
        ...
        const { size, concurrency, timeout, queue } = this.semaphore;
        this.application.console.error({
          msg: 'PROCEDURE semaphore fail',
          error,
          size,
          concurrency,
          timeout,
